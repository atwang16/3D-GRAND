...
def load_model_and_dataloader(
    ...
    data_loader = DataLoader(dataset, collate_fn=collator, batch_size=1, num_workers=0, shuffle=False)

    return tokenizer, model, data_loader


def get_model_response(
    model, tokenizer, data_loader, scene_id, user_input, max_new_tokens=50, temperature=0.2, top_p=0.9
):
    input_data = [
        {
            ...
            "conversations": [
                {
                    "from": "human",
                    "value": f"Ground these sentences: <refer_expression>{user_input}<refer_expression>\n<image>",
                }
            ],
