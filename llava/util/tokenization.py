...
def preprocess_multimodal(sources: Sequence[str], is_multimodal: bool, mm_use_im_start_end: bool) -> Dict:
...
def preprocess_llama_2_obj_identifier(
    ...
    assert input_ids.shape[0] == targets.shape[0] == 1, "Only support tokenization for one conversation at a time"
    ...
    if obj_context_feature_type == "vector":
        ...
        tokenized_obj_context = tokenizer(obj_context).input_ids[1:-1]  # strip the bos and eos tokens
